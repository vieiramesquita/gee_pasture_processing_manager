...
def run_classification(grid, decade):
    # Collections
    ...
    year_start = ee.Number(decade).subtract(1984)
    year_end = ee.Number(decade).add(11).subtract(1984)
    years_list = ee.List.sequence(1984, 2025).slice(year_start, year_end)
    ...
    def process_year(year):
        ...
        return classification.rename(ee.String('CY').cat(year.int().format()))

    classified_years = years_list.map(process_year)

    image = ee.Image(ee.ImageCollection(classified_years).toBands()).clip(classificationArea).multiply(10000).toInt()

    return classificationArea.geometry().bounds(), image

def submit_task(task_config):
    grid, decade = task_config["grid"], task_config["decade"]
    ROI, image = run_classification(grid, decade)
    
    decade_next = decade + 10
    if decade_next > 2025:
        decade_next = 2025
